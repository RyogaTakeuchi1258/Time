...
FutureScene::FutureScene(tnl::Vector3& pos, tnl::Vector3& obj_pos) {
	...
	button_ = std::make_shared<UI_FutureXbutton>();
}

void FutureScene::update(float delta_time) {
	SetFogEnable(true);
...
void FutureScene::draw() {
	SetFontSize(30);
	DrawStringEx(10, 10, -1, "未来シーン");
...
void FutureScene::updateOptionScreen(float delta_time) {
	if (!option_->update()) {
...
void FutureScene::updateXbuttonScreen(float delta_time) {
	button_->draw();
